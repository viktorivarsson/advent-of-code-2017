...
const splitSpace = (str: string): Array<string> => str.split(' ');
...
const uniqueByWords = (words: Array<string>): boolean => {
  // Get highest number of repeats for words
  const topOccurances = words.reduce((top: number, word, i, arr) => {
...
const numberOfValidPhrases = (lines: Array<string>): number => {
...
const uniqueByLetters = (words: Array<string>): boolean => {
  const sorted = words.map(sortLetters);

  // Get highest number of occurances
  const topOccurances = sorted.reduce((top: number, word, i, arr) => {
    const repeats = arr.filter(x => x === word).length;
...
const validPassphraseByLetters = (lines: Array<string>): number => {
