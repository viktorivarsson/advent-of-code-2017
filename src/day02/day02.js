...
const getCols = (row: string): Array<number> => row.split('\t').map(Number);

// Calculate checksum for rows given a function to be applied to every row
const checksum = (rows: Array<string>) => (fn: Function): number =>
  rows.reduce((sum: number, row) => (sum += fn(row)), 0);
...
const diffMinMax = (row: string): number => {
...
const dividedPartnerValue = (cols: Array<number>, i: number): number => {
  const curr = cols[i];
...
const getEvenDivided = (row: string): number => {
  const cols = getCols(row);
